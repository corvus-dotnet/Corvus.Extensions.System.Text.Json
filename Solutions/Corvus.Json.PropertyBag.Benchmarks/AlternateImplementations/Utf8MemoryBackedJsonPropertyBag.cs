// <copyright file="Utf8MemoryBackedJsonPropertyBag.cs" company="Endjin Limited">
...
/// <summary>
/// The first prototype System.Text.Json-based property bag, in which the underlying storage was
/// a <see cref="ReadOnlyMemory{T}"/> containing the raw UTF-8 data.
/// </summary>
/// <remarks>
/// This has been retained for benchmarking comparison purposes. When the decision was taken to use
/// a cloned <see cref="JsonElement"/> as the underlying storage (see ADR 0001), we compared the
/// memory use and performance of this implementation against the implementation we ultimately
/// chose. Instead of just discarding this prototype, we've moved into the benchmark project
/// to enable those measurements to be repeated, to verify that on future runtimes, the chosen
/// implementation continues to offer superior performance.
/// </remarks>
internal class Utf8MemoryBackedJsonPropertyBag : IPropertyBag, IEnumerable<(string Key, PropertyBagEntryType Type)>
{
    private static readonly byte[] EmptyObject = Encoding.UTF8.GetBytes("{}");

    /// <summary>
    /// Initializes a new instance of the <see cref="Utf8MemoryBackedJsonPropertyBag"/> class.
    /// </summary>
    ...
    public Utf8MemoryBackedJsonPropertyBag(ReadOnlyMemory<byte> json, JsonSerializerOptions serializerOptions)
    {
        this.RawJson = json;
    ...
    /// <param name="serializerOptions">The serializer settings to use for the property bag.</param>
    public Utf8MemoryBackedJsonPropertyBag(string json, JsonSerializerOptions serializerOptions)
